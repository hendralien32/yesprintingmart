  ...
  $.ajax({
    ...
    success: function (data) {
      if (data > 0) {
        ...
        $("#Alert_Val" + id).html(
          "<b style='color:red'> " + id + " sudah terdaftar</b>"
        );
      } else {
        $("#validasi_" + id).val("0");
        $("#Alert_Val" + id).html(
          "<b style='color:green'> " + id + " belum terdaftar</b>"
        );
      }
    },
  });
...
function submit(id) {
  ...
  var Special;
  if ($("#form_special").prop("checked") == true) {
    Special = "Y";
  } else {
    Special = "N";
  }
  ...
  if (EmailClient != "" && re.test(EmailClient) == false) {
    // this return result in boolean type
