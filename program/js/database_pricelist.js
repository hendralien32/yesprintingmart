...
function ChangeKodeBrg() {
  ...
  } else if (kode_barang[0] == "large format" || kode_barang[0] == "xuli") {
  ...
  $.ajax({
    ...
    success: function (data) {
      if (data > 0) {
        $("#validasi_" + id).val(data);
        $("#Alert_Val" + id).html(
          "<b style='color:red'> Pricelist bahan sudah terdaftar</b>"
        );
      } else {
        $("#validasi_" + id).val(data);
        $("#Alert_Val" + id).html(
          "<b style='color:green'> Pricelist bahan belum terdaftar</b>"
        );
      }
    },
  });
